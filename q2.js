...
function division(array, divSize) {
    const result = [];

    for (let i = 0; i < array.length; i += divSize) {
        const part = array.slice(i, i + divSize);
        result.push(part);
    }
...
const divisionArray = division(Array.from({length:80}, (_, index) => index + 1), 5);
